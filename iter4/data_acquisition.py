"""
label_wrist_smoothed_2class.py
Real-time EMG plot + continuous labeling: REST / ACTIVE (non-rest)
...
Y_MIN = 0
Y_MAX = 1024
OUTPUT_CSV = "labeled_stream_2class.csv"
...
# current label (REST by default)
current_label = "REST"   # will be changed with keys r/a
    ...
    while True:
        try:
            if ser.in_waiting > 0:
                ...
                try:
                    if ',' in raw_line:
                        # If you're using the EMG envelope sketch (raw,envelope),
                        # take the second value (envelope) for labeling.
...
ax.set_title(
    ...
    "r=REST, a=ACTIVE, t=toggle smoothing"
)
...
print("Controls: r=REST, a=ACTIVE, t=toggle smoothing")
