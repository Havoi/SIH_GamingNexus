#!/usr/bin/env python3
"""
EMG realtime predictor with Space trigger, voting, consecutive requirement and refractory
...
- Fires Space key when:
      (votes indicate ACTIVE) AND (CONSECUTIVE_ACTIVE_REQUIRED met)
  Then ignores further firings for REFRACTORY_PERIOD_SEC seconds.
- Prints decisions and metadata to terminal
"""
...
import pyautogui  # make sure installed: pip install pyautogui
...
# NEW: number of consecutive ACTIVE windows required
CONSECUTIVE_ACTIVE_REQUIRED = 5

# Refractory after firing (seconds)
REFRACTORY_PERIOD_SEC = 2

# how long to hold space (seconds)
SPACE_HOLD_SEC = 0.1
...
def send_space():
    """Send a short Space keypress using pyautogui."""
    try:
        prev_pause = pyautogui.PAUSE
        pyautogui.PAUSE = 0
        pyautogui.keyDown('space')
        time.sleep(SPACE_HOLD_SEC)
        pyautogui.keyUp('space')
        pyautogui.PAUSE = prev_pause
    except Exception as e:
        print("Warning: failed to send Space key:", e)
...
def main():
    ...
    # new state vars
    consec_active_count = 0
    last_active_time = None  # timestamp of last Space press
        ...
        while True:
            if ser.in_waiting > 0:
                ...
                if len(buffer) == WIN_SIZE and (sample_count % STEP_SIZE == 0):
                    ...
                    votes.append(vote)
                    votes_sum = sum(votes)
                    vote_active = (votes_sum >= REQUIRED_VOTES and len(votes) == VOTE_WINDOW)

                    # consecutive ACTIVE logic
                    if p_used >= PROB_THRESHOLD:
                        consec_active_count += 1
                    else:
                        consec_active_count = 0

                    consec_condition = (consec_active_count >= CONSECUTIVE_ACTIVE_REQUIRED)

                    # final combined decision: require both vote condition and consecutive condition
                    is_active = vote_active and consec_condition
                    ...
                    if is_active:
                        ...
                        if last_active_time is None or (now - last_active_time) >= REFRACTORY_PERIOD_SEC:
                            ...
                            # reset vote & consecutive counters so we don't retrigger immediately
                            votes.clear()
                            consec_active_count = 0
                        else:
                            suppressed = True
                    ...
                    if fired:
                        print(f"[{t}] p_raw={p_active:.3f} p_used={p_used:.3f} votes={votes_sum}/{VOTE_WINDOW} consec={consec_active_count} => ACTIVE (SPACE SENT)")
                    elif suppressed:
                        rem = REFRACTORY_PERIOD_SEC - (now - last_active_time)
                        print(f"[{t}] p_raw={p_active:.3f} p_used={p_used:.3f} votes={votes_sum}/{VOTE_WINDOW} => ACTIVE (suppressed, {rem:.2f}s left)")
                    else:
                        label_str = "ACTIVE" if is_active else "REST"
                        print(f"[{t}] p_raw={p_active:.3f} p_used={p_used:.3f} votes={votes_sum}/{len(votes)} consec={consec_active_count} => {label_str}")

                    last_printed_state = label_str if 'label_str' in locals() else ( "ACTIVE" if fired else "REST" )
