...
# Labels config
REST_LABEL = "REST"
# Any label that is NOT REST will be treated as "inside a gesture"
ACTIVE_LABELS = {"UP", "DOWN"}   # adjust if your labels differ

# How many consecutive REST predictions before we consider the gesture finished
MIN_REST_FRAMES = 3
# ====================================================
...
def main():
    ...
    # --- Gesture-level state ---
    gesture_state = "RESTING"      # or "IN_GESTURE"
    rest_frames = 0                # count of consecutive REST frames
    current_gesture_label = None   # UP or DOWN for this gesture
    # ---------------------------
        ...
        while True:
            ...
            if ser.in_waiting > 0:
                ...
                if len(buffer) == WIN_SIZE and (sample_count % STEP_SIZE == 0):
                    ...
                    # ------ Gesture logic: print ONE UP + ONE REST per action ------
                    is_active = label in ACTIVE_LABELS

                    if gesture_state == "RESTING":
                        if is_active:
                            # Gesture starts: REST -> ACTIVE
                            current_gesture_label = label
                            t = time.strftime("%H:%M:%S")
                            print(f"[{t}] {current_gesture_label}")
                            gesture_state = "IN_GESTURE"
                            rest_frames = 0
                        # else: still REST, do nothing

                    else:  # gesture_state == "IN_GESTURE"
                        if not is_active:
                            # we see REST; count consecutive REST frames
                            rest_frames += 1
                            if rest_frames >= MIN_REST_FRAMES:
                                # Gesture ends: ACTIVE -> REST
                                t = time.strftime("%H:%M:%S")
                                print(f"[{t}] {REST_LABEL}")
                                gesture_state = "RESTING"
                                rest_frames = 0
                                current_gesture_label = None
                        else:
                            # still active: reset rest counter
                            rest_frames = 0
                    # ----------------------------------------------------------------
